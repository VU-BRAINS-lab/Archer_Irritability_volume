...
lm.tores.primary <- lm(Y.primary ~ age_b + FEMALE_b + NHWHITE_b + AFRICAN_b + HISPANIC_b + 
                         COIL2_b + COIL3_b + COIL4_b + COIL5_b + parent_education_b + medication_b + fsqc_qu_motion_b, data = irritability.data)
lm.residuals.primary <- lm.tores.primary$residuals
#add residuals matrix to the data frame and rename the data frame
main.ed.med.motion.residual <- irritability.data %>% spread_residuals(lm.tores.primary)
#condense new data frame to just sub_num, weight, and residual column
main.ed.med.motion.residual <- main.ed.med.motion.residual[c(grep("subnum_char_b|wt_NR_mwacs_b|lm.tores.primary", names(main.ed.med.motion.residual)))]
#omit cases with NAs in row
main.ed.med.motion.residual <- na.omit(main.ed.med.motion.residual)
#save csv file with sub ID, weight, and all residuals
write.csv(main.ed.med.motion.residual,"main.ed.med.motion.residual.csv", row.names = FALSE)
main.ed.med.motion.residual<-read.csv('main.ed.med.motion.residual.csv')

#rename
main.ed.med.motion.residual <- main.ed.med.motion.residual %>% rename("vol_1_r" = "lm.tores.primary.vol_1", 
...
main.ed.med.motion.residual<-read.csv('main.ed.med.motion.residual.csv')
main.ed.med.motion.residual<- (main.ed.med.motion.residual[c(1,3:90)])
main.ed.med.motion.residual<- merge(irritability.data, main.ed.med.motion.residual, by = "subnum_char_b")
# replace NA with .
main.ed.med.motion.residual[is.na(main.ed.med.motion.residual)]<-"."
#write_csv(main.data, "irritability_new_data.csv")
write.dat(main.ed.med.motion.residual, "main.ed.med.motion.residual")
